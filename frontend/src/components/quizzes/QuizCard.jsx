...
export default function QuizCard({ quiz, onDelete }) {
  return (
    <div className="group relative bg-white/80 backdrop-blur-xl border-2 border-slate-200 hover:border-orange-300 rounded-2xl p-4 transition-all duration-200 hover:shadow-lg hover:shadow-orange-500/10 flex flex-col justify-between">
      <button
        onClick={(e) => {
          e.stopPropagation();
          onDelete(quiz);
      ...
      <div className="space-y-4">
        ...
        <div className="inline-flex items-center gap-1.5 py-1 rounded-lg text-xs font-semibold">
          <div className="flex items-center gap-1.5 bg-orange-50 border border-orange-200 rounded-lg px-3 py-1">
            <Award className="w-3.5 h-3.5 text-orange-600" strokeWidth={2.5} />
            <span className="text-orange-700">Score: {quiz?.score}</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 pt-2 border-t border-slate-100">
          <div className="px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-lg">
            <span className="text-sm font-semibold Otext-slate-700">
      ...
      <div className="mt-2 pt-4 border-t border-slate-100">
        {quiz?.userAnswers?.length > 0 ? (
          ...
          <Link to={`/quizzes/${quiz._id}`}>
            <button className="group/btn relative w-full h-11 bg-linear-to-r from-orange-500 to-orange-500 hover:from-orange-600 hover:to-orange-600 text-sm text-white font-semibold rounded-xl transition-all duration-200 shadow-lg shadow-orange-500/25 active:scale-95 overflow-hidden">
